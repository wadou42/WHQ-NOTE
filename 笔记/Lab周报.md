...
opt有影响，但是不如code影响大。不同的code因为在O3的情况下已经效果“比较好了”，或者说被内联等操作导致执行时间非常短，导致加速比较非常小，并且方差也较小。同理一些code执行时间很长，也会导致speedup很大。 

IDEA： 数据集筛选的时候，筛选掉这些数据？？但是这样会忽略掉一些flag，例如inline很可能不被考虑到。（这个还没试）



- [ ] 检验一个模型在更小的范围内是否可行（回归任务）？
我们按照代码表征对code进行聚类，然后在每一类中按照9:1划分数据集，效果依然一般。但是这里聚类使用的是KMEANS聚类，我们的code表征之后维度是768，效果可能不是太好。后续考虑使用PCA降维再聚类试一试。


### NEW TODO
- [ ] 正负标签划分，使用一个其他的值作为划分。比如大于0.9 设置为正样例，小于0.9设为负样例
- [ ] 数据集筛选方式筛选掉加速比均值异常的数据
- [ ] PCA降维后再聚类试一试